# QCViz-MCP Enterprise v6 UI 다이어트 및 이력 복원 마스터 프롬프트

**작업 지시서 (Prompt):**
당신은 실리콘밸리 탑티어 풀스택 아키텍트이자 UI/UX 마스터입니다. 
제공된 QCViz-MCP의 프론트엔드/백엔드 최신 코드를 기반으로, 시스템을 **'전문가용 엔터프라이즈 SaaS'** 수준으로 완전히 탈바꿈시켜야 합니다. 

아래의 문제점들을 100% 해결하는 **완벽한 Full Code**를 작성해주세요.

### 🎯 [작업 목표 1: 뷰어 패널 UI 대청소 (다이어트)]
1. 현재 `index.html`의 `<section class="viewer-panel">` 내부를 보면, 쓸데없는 텍스트(예: "Structure, orbital isosurfaces..."), 불필요한 `hero-info` 메트릭들, 중복된 설명들이 너무 많아 화면만 차지하고 있습니다.
2. 전문가가 보기에 불필요하고 거추장스러운 텍스트와 UI 요소를 과감하게 싹 다 지워주세요. (미니멀리즘 극대화)
3. 3D Viewer 캔버스와 꼭 필요한 기능 버튼(컨트롤바)만 깔끔하고 직관적으로 남겨주세요.

### 🎯 [작업 목표 2: 기능 버튼(인터랙션) 완벽 정상화]
1. 현재 Style(Ball+Stick, Stick, Sphere) 변경, Show Orbital/ESP 토글, Atom/Charge 레이블 토글 등의 버튼이 제대로 동작하지 않거나 버그(예: 이전 렌더링이 안 지워짐)가 많습니다.
2. `viewer.js`와 `index.html`을 수정하여, 모든 기능 버튼이 3Dmol.js 뷰어와 데이터(result payload)에 즉각적이고 정확하게 연동되도록 로직을 완벽히 수선해주세요.
3. 오비탈이나 ESP를 전환할 때 이전 서피스(Surface) 객체를 깔끔하게 메모리에서 해제(Dispose)하는 누수 방지 로직도 적용하세요.

### 🎯 [작업 목표 3: Job History (이력 센터) 구축 및 상태 동기화]
1. 지금까지 수행한 계산 목록(Jobs)을 한눈에 볼 수 있는 이력 패널을 우측에 세련되게 디자인해주세요.
2. **[핵심]** 특정 이력을 클릭하면, 해당 계산의 결과 데이터(JSON)와 3D 렌더링 상태(오비탈, ESP, 컨트롤바 수치 등)가 과거 시점 그대로 3D 뷰어와 결과 탭에 100% 완벽하게 복원되어야 합니다.
3. 이를 위해 `chat.js`, `results.js`, `viewer.js` 간에 쪼개져 있는 상태(State)를 일원화하거나 완벽히 동기화하는 이벤트 기반 구조를 적용하세요.

### 🎯 [작업 목표 4: 엔터프라이즈 미학 (Design System)]
1. 데스크톱 환경에 최적화된 엔터프라이즈급 UI/UX를 위해 `style.css`를 대폭 강화하세요.
2. Glassmorphism, 부드러운 호버/트랜지션 애니메이션, 여백을 활용한 고급스러운 그리드 배치를 적용하세요.

### 🚨 [출력 규칙]
*   수정이 필요한 모든 파일(`index.html`, `style.css`, `viewer.js`, `results.js`, `chat.js`)에 대해 **복사-붙여넣기가 가능한 수준의 완벽한 전체 코드(Full Code)**를 마크다운 블록으로 분리하여 제공하세요.
