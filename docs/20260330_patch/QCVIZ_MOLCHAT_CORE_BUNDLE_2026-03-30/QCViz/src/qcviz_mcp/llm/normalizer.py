...
def analyze_follow_up_request(text: str) -> Dict[str, Any]:
    ...
    primary_candidate = str(structure_analysis.get("primary_candidate") or "").strip()
    explicit_structure = bool(primary_candidate) and not (
        bool(_FOLLOW_UP_PLACEHOLDER_RE.fullmatch(primary_candidate))
        or detect_task_hint(primary_candidate)
    )
    lower = raw.lower()
    ...
    if _FOLLOW_UP_ORBITAL_RE.search(search_text):
        ...
        if not follow_up_mode and not explicit_structure and has_continuation_cue:
        ...
        if _FOLLOW_UP_SAME_STRUCTURE_RE.search(search_text) or not explicit_structure:
    ...
    if _FOLLOW_UP_SAME_STRUCTURE_RE.search(search_text) and not follow_up_mode:
...
def normalize_user_text(text: str) -> Dict[str, Any]:
    ...
    return {
        ...
        "maybe_structure_hint": None if semantic_info.get("semantic_descriptor") or query_routing.get("chat_only") or structure_analysis.get("multi_molecule") or len(composite_structures) >= 2 else (
            (
                structure_hypotheses.get("primary_candidate")
                if _is_plausible_structure_candidate(structure_hypotheses.get("primary_candidate"))
                else None
