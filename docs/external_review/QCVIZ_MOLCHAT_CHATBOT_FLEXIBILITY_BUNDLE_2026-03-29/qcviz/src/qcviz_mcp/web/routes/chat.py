...
def _current_missing_slots(plan: Mapping[str, Any], payload: Mapping[str, Any]) -> List[str]:
    ...
    if not (payload.get("structure_query") or payload.get("xyz") or payload.get("atom_spec") or payload.get("structures")):
...
async def _molchat_interpret_candidates(query: str) -> List[Dict[str, Any]]:
    ...
    if not out or interpreted_sources == {"molchat_search_fallback"}:
        ...
        if llm_grounded:
            return _merge_structure_suggestions(llm_grounded, out)
    else:
        logger.info(
...
async def _build_clarification_form(
    ...
    *,
    ...
    if mode == "disambiguation":
        ...
        title = "?ㅻ챸??湲곕컲 ?꾨낫瑜??뺤씤??二쇱꽭??/ Confirm candidates from the description"
        message = "?낅젰?섏떊 ?ㅻ챸??湲곕컲?쇰줈 MolChat?먯꽌 援ъ“?붾맂 ?꾨낫瑜??뺣━?덉뒿?덈떎."
    elif mode == "discovery":
    ...
    *,
    ...
    if structure_choice:
        if structure_choice == "custom" and structure_custom:
            ...
            merged["structure_query"] = structure_choice
    ...
    while True:
        ...
        if snap.get("status") in TERMINAL_STATES:
            ...
            await _ws_send(websocket, "result", session_id=session_id, job=terminal,
                           result=result, summary=_result_summary(result), turn_id=turn_id)
            return
...
async def websocket_chat(websocket: WebSocket) -> None:
    ...
    try:
        while True:
            ...
            plan_intent = plan.get("intent", "") if isinstance(plan, dict) else getattr(plan, "intent", "")
            chat_response = (plan.get("chat_response", "") if isinstance(plan, dict) else getattr(plan, "chat_response", "")) or ""
            if plan_intent == "chat":
                # If Gemini didn't provide chat_response, make a direct call
                if not chat_response:
                    from qcviz_mcp.web.routes.compute import get_qcviz_agent
                    agent = get_qcviz_agent()
                    if agent and hasattr(agent, 'chat_direct'):
                        history = session_state.get("chat_history", [])
                        chat_response = agent.chat_direct(user_message or "", context=history)
                if chat_response:
