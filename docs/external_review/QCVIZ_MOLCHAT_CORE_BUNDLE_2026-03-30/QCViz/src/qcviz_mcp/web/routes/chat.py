...
def _build_semantic_chat_response(
    ...
    if len(candidates) == 1:
        ...
        if lead_token:
            first = f"`{lead_token}`는 보통 **{name}**을 의미합니다."
        elif question_like:
            first = f"입력하신 표현은 보통 **{name}**을 의미합니다."
        else:
        ...
        tail = "원하시면 이 분자에 대해 HOMO/LUMO, ESP, 구조 최적화 같은 계산을 이어서 진행할 수 있습니다."
        if details:
            return first + "\n\n- " + "\n- ".join(details) + "\n\n" + tail
        return first + "\n\n" + tail

    formatted = [_format_grounded_candidate(item) for item in candidates[:3]]
    first = f"입력하신 표현은 여러 후보로 해석될 수 있습니다. 가장 가까운 후보는 다음과 같습니다.\n\n- " + "\n- ".join(formatted)
    if lead_token:
        first = f"`{lead_token}`는 여러 물질을 가리킬 수 있습니다.\n\n- " + "\n- ".join(formatted)
    return first + "\n\n원하시는 분자를 이름이나 SMILES로 다시 지정해 주시면 그 기준으로 계산을 이어가겠습니다."
