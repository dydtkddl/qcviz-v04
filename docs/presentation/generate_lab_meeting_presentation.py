...
def add_title_subtitle(slide, title, subtitle=""):
    title_box = slide.shapes.add_textbox(TITLE_LEFT, TITLE_TOP, TITLE_WIDTH, Inches(0.42))
    tf = title_box.text_frame
    p = tf.paragraphs[0]
    p.alignment = PP_ALIGN.LEFT
    run = p.add_run()
    run.text = title
    apply_font(run, 34, DARK_NAVY, bold=True)
...
def add_label_chip(slide, left, top, width, text, fill_color=PRIMARY_BLUE, text_color=WHITE):
    ...
    run = p.add_run()
    run.text = text
    apply_font(run, 11, text_color, bold=True)
    return chip
    ...
    for r in range(rows):
        for c in range(cols):
            ...
            for p in cell.text_frame.paragraphs:
                p.alignment = PP_ALIGN.LEFT
                for run in p.runs:
                    apply_font(run, 12 if r > 0 else 13, text_color, bold=bold)
    ctx["tables"] += 1
    return table
...
def render_title_slide(slide, ctx):
    ...
    apply_font(r, 30, DARK_NAVY, bold=True)
    p2 = tf.add_paragraph()
    r2 = p2.add_run()
...
def build_presentation():
    ...
    output_path = Path("docs/presentation/LAB_MEETING_PRESENTATION.pptx")
