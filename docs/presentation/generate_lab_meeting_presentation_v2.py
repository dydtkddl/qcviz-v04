...
def add_title_subtitle(slide, title, subtitle=""):
    ...
    title_box = slide.shapes.add_textbox(TITLE_LEFT, TITLE_TOP, TITLE_WIDTH, Inches(0.48))
    ...
    if subtitle:
        sub_box = slide.shapes.add_textbox(TITLE_LEFT, SUBTITLE_TOP, TITLE_WIDTH, Inches(0.32))
        tf = sub_box.text_frame
        p = tf.paragraphs[0]
        p.alignment = PP_ALIGN.LEFT
        run = p.add_run()
        run.text = subtitle
        apply_font(run, 19, DARK_GRAY)
...
def render_section_divider(slide, title, subtitle):
...
def render_title_slide(slide, ctx):
    title_box = slide.shapes.add_textbox(Inches(0.7), Inches(0.8), Inches(7.2), Inches(1.2))
    tf = title_box.text_frame
    p = tf.paragraphs[0]
    r = p.add_run()
    r.text = "Quantum Chemistry Automation Platform"
    apply_font(r, 32, DARK_NAVY, bold=True)
    p2 = tf.add_paragraph()
    r2 = p2.add_run()
    r2.text = "LLM + MCP + PySCF for conversational computation"
    apply_font(r2, 20, DARK_GRAY)
    ...
    add_label_chip(slide, Inches(0.72), Inches(3.15), Inches(1.6), "Refined V2", fill_color=PRIMARY_RED)
    add_label_chip(slide, Inches(0.72), Inches(3.55), Inches(2.2), "Lab Meeting Deck", fill_color=TEAL)
    add_label_chip(slide, Inches(3.05), Inches(3.55), Inches(2.4), "Browser-Based UI", fill_color=PRIMARY_BLUE)
    add_label_chip(slide, Inches(5.58), Inches(3.55), Inches(1.7), "Open Source", fill_color=PRIMARY_GREEN)
...
def build_presentation():
    ...
    slides = [
        ...
        ("divider1", lambda s, c: render_section_divider(s, "Why This Problem Matters", "Scientific need, workflow friction, and the current research gap"), ["This divider marks the transition from opening claims to the motivation section."]),
        ...
        ("divider2", lambda s, c: render_section_divider(s, "Platform Concept", "How the system converts a natural-language request into a compute-backed result"), ["Move from motivation into platform design."]),
        ...
        ("divider3", lambda s, c: render_section_divider(s, "Capabilities", "What the current system can already calculate and present"), ["Transition from architecture into concrete user-visible features."]),
        ...
        ("divider4", lambda s, c: render_section_divider(s, "Demo And Evidence", "How to show the platform convincingly without unsupported benchmark claims"), ["Transition from capability description to presentation strategy and evidence framing."]),
        ...
        ("divider5", lambda s, c: render_section_divider(s, "Impact And Outlook", "Why the platform matters for the lab, and how it could evolve"), ["Transition from current-state evidence into implications and future work."]),
        ...
        ("divider6", lambda s, c: render_section_divider(s, "Appendix", "Backup slides for architecture, runtime, and evaluation"), ["Transition into backup material."]),
    ...
    output_path = Path("docs/presentation/LAB_MEETING_PRESENTATION_V2.pptx")
    output_path.parent.mkdir(parents=True, exist_ok=True)
    prs.save(output_path)
    reopened = Presentation(output_path)
    return output_path, tracker, len(reopened.slides)


def main():
    output_path, tracker, reopened_count = build_presentation()
