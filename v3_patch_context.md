    ...
    switch (type) {
      ...
      case "job_created": case "job_queued":
        ...
        App.upsertJob({
            ...
            basis_set: jobSnap.basis_set || jobSnap.basis || msg.basis_set || msg.basis || (msg.payload ? (msg.payload.basis_set || msg.payload.basis) : "") || "",
        });
      ...
      case "status": case "step": case "stage": case "computing": case "running":
        ...
        if (typeof progress === "number") {
          ...
          var pct = progress <= 1.0 && progress >= 0 ? progress * 100 : progress;
  ...
  function bindEvents() {
    ...
    if (dom.$sliderEspDensIso) {
      dom.$sliderEspDensIso.addEventListener("input", function () {
        ...
        if (dom.$lblEspDensIso) dom.$lblEspDensIso.textContent = state.espDensityIso.toFixed(4);
      });
      dom.$sliderEspDensIso.addEventListener("change", function () { reRenderCurrentSurface(); saveViewerSnapshot(); });
    }
...
(function () {
  ...
  function normalizeResult(raw) {
    ...
    if (
      ...
      var sorted = r.orbitals.slice().sort(function (a, b) {
        var ai = typeof a.zero_based_index === "number" ? a.zero_based_index : 0;
        var bi = typeof b.zero_based_index === "number" ? b.zero_based_index : 0;
        return ai - bi;
      });
  ...
  function getAvailableTabs(r) {
    ...
    var hasMulliken = Array.isArray(r.mulliken_charges) ? r.mulliken_charges.length > 0
      : (r.mulliken_charges && typeof r.mulliken_charges === "object" && Object.keys(r.mulliken_charges).length > 0);
    var hasLowdin = Array.isArray(r.lowdin_charges) ? r.lowdin_charges.length > 0
      : (r.lowdin_charges && typeof r.lowdin_charges === "object" && Object.keys(r.lowdin_charges).length > 0);
